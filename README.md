...
| `etch_screenshot` | PNG of the tab (`hide_chrome: true` for clean shots) — closes the visual feedback loop |
...
- **`EtchBridge` is an interface.** CDP is the v1 transport; a WordPress-plugin/WebSocket relay can slot in later without touching any tool.
